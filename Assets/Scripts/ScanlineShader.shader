  ...
  SubShader {
    Pass {
      ...
      float4 frag(v2f_img i) : COLOR {
        ...
        // Modified
        _Factor = _Texcoord.y * _TextureSize.y * _OutputSize.y / _InputSize.y;
        ...
        // Modified
        float4 col = tex2D(_MainTex, xy);
        float4 col2 = tex2D(_MainTex, xy + float2(_One.x, 0.0f));
        float4 weights = ScanlineWeights(uvratio.x, col);
        float4 weights2 = ScanlineWeights(1.0f - uvratio.x, col2);
